...
-- Then modify the policies to allow inserts
ALTER TABLE users DISABLE ROW LEVEL SECURITY;
-- OR
CREATE POLICY "Allow all insert operations" ON users FOR INSERT WITH CHECK (true);
...
-- Check existing policies for orders table
SELECT * FROM pg_policies WHERE tablename = 'orders';

-- Disable RLS for orders table for testing
ALTER TABLE orders DISABLE ROW LEVEL SECURITY;

-- Or create a permissive policy for orders
CREATE POLICY "Allow insert to orders" ON orders
FOR INSERT
WITH CHECK (true);

-- Enable Row Level Security (Note: This contradicts the DISABLE statements above, 
-- so you may want to comment this out during initial development)
